        ...
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ flexGrow: 1 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          <View style={{ flex: 1, justifyContent: 'space-between', paddingHorizontal: 24, paddingTop: 80, paddingBottom: 40 }}>
            ...
            <View style={{ marginBottom: 32 }}>
              ...
              <Text style={{ color: '#fff', fontSize: 40 * fontScale, fontWeight: '800', lineHeight: 46, letterSpacing: -1 }}>Bem-vindo{'\n'}de volta.</Text>
              <Text style={{ color: 'rgba(255,255,255,0.65)', fontSize: 15 * fontScale, marginTop: 8 }}>Faça login para continuar</Text>
            </View>
            ...
            <View style={{ backgroundColor: '#fff', borderRadius: 28, padding: 28, shadowColor: '#000', shadowOffset: { width: 0, height: 12 }, shadowOpacity: 0.2, shadowRadius: 24, elevation: 16 }}>
              ...
              <View style={{ marginBottom: 16 }}>
                <Text style={{ color: '#374151', fontSize: 10 * fontScale, fontWeight: '800', letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 8 }}>Email</Text>
                <View style={{ backgroundColor: viewModel.emailError ? '#FEF2F2' : '#F9FAFB', borderWidth: 1.5, borderColor: viewModel.emailError ? '#FCA5A5' : '#E5E7EB', borderRadius: 14, paddingHorizontal: 16, paddingVertical: 14 }}>
                  <TextInput style={{ color: '#111827', fontSize: 15 * fontScale }} placeholder="seu@email.com" placeholderTextColor="#9CA3AF" value={viewModel.email} onChangeText={viewModel.setEmail} keyboardType="email-address" autoCapitalize="none" autoComplete="email" />
                </View>
              ...
              <View style={{ marginBottom: 10 }}>
                <Text style={{ color: '#374151', fontSize: 10 * fontScale, fontWeight: '800', letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 8 }}>Senha</Text>
                <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: viewModel.passwordError ? '#FEF2F2' : '#F9FAFB', borderWidth: 1.5, borderColor: viewModel.passwordError ? '#FCA5A5' : '#E5E7EB', borderRadius: 14, paddingHorizontal: 16, paddingVertical: 14 }}>
                  <TextInput style={{ flex: 1, color: '#111827', fontSize: 15 * fontScale }} placeholder="••••••••" placeholderTextColor="#9CA3AF" value={viewModel.password} onChangeText={viewModel.setPassword} secureTextEntry={!viewModel.showPassword} autoCapitalize="none" />
              ...
              <TouchableOpacity onPress={onForgotPassword} style={{ alignSelf: 'flex-end', marginBottom: 24 }}>
                <Text style={{ color: '#FF662A', fontSize: 13 * fontScale, fontWeight: '600' }}>Esqueceu a senha?</Text>
              </TouchableOpacity>
              ...
              <TouchableOpacity onPress={handleLogin} disabled={viewModel.loading} activeOpacity={0.85} style={{ opacity: viewModel.loading ? 0.65 : 1 }}>
                <LinearGradient colors={['#FCA537', '#FF662A']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={{ borderRadius: 14, paddingVertical: 17, alignItems: 'center', justifyContent: 'center' }}>
                  {viewModel.loading ? <ActivityIndicator color="#fff" size="small" /> : <Text style={{ color: '#fff', fontSize: 16 * fontScale, fontWeight: '700', letterSpacing: 0.5 }}>Entrar</Text>}
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 28, marginBottom: 20 }}>
                ...
                <Text style={{ color: '#9CA3AF', fontSize: 13 * fontScale, marginHorizontal: 12 }}>Ou</Text>
                <View style={{ flex: 1, height: 1, backgroundColor: '#E5E7EB' }} />
              </View>

              <View style={{ flexDirection: 'row', justifyContent: 'center' }}>
                <Text style={{ color: '#6B7280', fontSize: 14 * fontScale }}>Realize seu </Text>
                <TouchableOpacity onPress={onRegister}>
                  <Text style={{ color: '#FF662A', fontSize: 14 * fontScale, fontWeight: '700' }}>cadastro</Text>
                </TouchableOpacity>
              </View>
            </View>
