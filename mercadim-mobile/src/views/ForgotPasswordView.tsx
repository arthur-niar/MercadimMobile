...
export const ForgotPasswordView: React.FC<ForgotPasswordViewProps> = ({ onBack, onSuccess }) => {
  ...
  const renderEmailStep = () => (
    <>
      <Text style={{ color: '#111827', fontSize: 22 * fontScale, fontWeight: '800', marginBottom: 6 }}>Recuperar senha</Text>
      <Text style={{ color: '#6B7280', fontSize: 13 * fontScale, marginBottom: 24 }}>Digite seu email para receber o código</Text>
      ...
      <View style={{ marginBottom: 28 }}>
        <Text style={getFieldLabel(fontScale)}>Email</Text>
        <View style={inputBox(!!viewModel.emailError)}>
          <TextInput style={getInputText(fontScale)} placeholder="seu@email.com" placeholderTextColor="#9CA3AF" value={viewModel.email} onChangeText={viewModel.setEmail} keyboardType="email-address" autoCapitalize="none" />
        </View>
        {viewModel.emailError ? <Text style={getErrText(fontScale)}>{viewModel.emailError}</Text> : null}
      </View>
      <GradientButton onPress={viewModel.requestPasswordReset} loading={viewModel.loading} label="Enviar código" fontScale={fontScale} />
    </>
  );

  const renderCodeStep = () => (
    <>
      <Text style={{ color: '#111827', fontSize: 22 * fontScale, fontWeight: '800', marginBottom: 6 }}>Digite o código</Text>
      ...
      <View style={{ marginBottom: 28 }}>
        <Text style={getFieldLabel(fontScale)}>Código (6 dígitos)</Text>
        <View style={inputBox(!!viewModel.codeError)}>
          <TextInput style={{ ...getInputText(fontScale), textAlign: 'center', letterSpacing: 8, fontWeight: '700', fontSize: 22 * fontScale }} placeholder="000000" placeholderTextColor="#D1D5DB" value={viewModel.code} onChangeText={viewModel.setCode} keyboardType="number-pad" maxLength={6} />
        </View>
        {viewModel.codeError ? <Text style={getErrText(fontScale)}>{viewModel.codeError}</Text> : null}
      </View>
      <GradientButton onPress={viewModel.verifyCode} loading={viewModel.loading} label="Verificar código" fontScale={fontScale} />
      <TouchableOpacity onPress={viewModel.requestPasswordReset} style={{ paddingVertical: 14, alignItems: 'center' }}>
        <Text style={{ color: '#FF662A', fontSize: 14 * fontScale, fontWeight: '600' }}>Reenviar código</Text>
      </TouchableOpacity>
    </>
  );
  ...
  const renderNewPasswordStep = () => (
    <>
      <Text style={{ color: '#111827', fontSize: 22 * fontScale, fontWeight: '800', marginBottom: 6 }}>Nova senha</Text>
      <Text style={{ color: '#6B7280', fontSize: 13 * fontScale, marginBottom: 24 }}>Crie uma senha com no mínimo 6 caracteres</Text>
      {viewModel.generalError ? (
        <View style={{ backgroundColor: '#FEF2F2', borderLeftWidth: 3, borderLeftColor: '#EF4444', borderRadius: 10, padding: 14, marginBottom: 20 }}>
          <Text style={{ color: '#B91C1C', fontSize: 13 * fontScale, fontWeight: '500' }}>{viewModel.generalError}</Text>
        </View>
      ...
      <View style={{ marginBottom: 16 }}>
        <Text style={getFieldLabel(fontScale)}>Nova senha</Text>
        <View style={{ ...inputBox(!!viewModel.passwordError), flexDirection: 'row', alignItems: 'center' }}>
          <TextInput style={{ ...getInputText(fontScale), flex: 1 }} placeholder="Sua nova senha" placeholderTextColor="#9CA3AF" value={viewModel.newPassword} onChangeText={viewModel.setNewPassword} secureTextEntry={!viewModel.showPassword} autoCapitalize="none" />
          <TouchableOpacity onPress={viewModel.togglePasswordVisibility}>
            <Text style={{ color: '#FF662A', fontSize: 13 * fontScale, fontWeight: '600' }}>{viewModel.showPassword ? 'Ocultar' : 'Mostrar'}</Text>
          </TouchableOpacity>
        </View>
        {viewModel.passwordError ? <Text style={getErrText(fontScale)}>{viewModel.passwordError}</Text> : null}
      </View>
      <View style={{ marginBottom: 28 }}>
        <Text style={getFieldLabel(fontScale)}>Confirmar senha</Text>
        <View style={inputBox(!!viewModel.confirmPasswordError)}>
          <TextInput style={getInputText(fontScale)} placeholder="Confirme sua senha" placeholderTextColor="#9CA3AF" value={viewModel.confirmPassword} onChangeText={viewModel.setConfirmPassword} secureTextEntry={!viewModel.showPassword} autoCapitalize="none" />
        </View>
        {viewModel.confirmPasswordError ? <Text style={getErrText(fontScale)}>{viewModel.confirmPasswordError}</Text> : null}
      </View>
      <GradientButton onPress={viewModel.verifyResetCode} loading={viewModel.loading} label="Redefinir senha" fontScale={fontScale} />
    </>
  );
  ...
  const renderSuccessStep = () => (
    <View style={{ alignItems: 'center', paddingVertical: 16 }}>
      ...
      <Text style={{ color: '#111827', fontSize: 22 * fontScale, fontWeight: '800', marginBottom: 8, textAlign: 'center' }}>Senha redefinida!</Text>
      ...
      <TouchableOpacity onPress={onSuccess} activeOpacity={0.85} style={{ width: '100%' }}>
        <LinearGradient colors={['#FCA537', '#FF662A']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={{ borderRadius: 14, paddingVertical: 17, alignItems: 'center', justifyContent: 'center' }}>
          <Text style={{ color: '#fff', fontSize: 16 * fontScale, fontWeight: '700', letterSpacing: 0.5 }}>Fazer login</Text>
        </LinearGradient>
      </TouchableOpacity>
    </View>
  );
        ...
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ flexGrow: 1 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          <View style={{ flex: 1, paddingHorizontal: 24, paddingTop: 64, paddingBottom: 40 }}>
            <TouchableOpacity onPress={handleBack} style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 32 }}>
              <Text style={{ color: 'rgba(255,255,255,0.85)', fontSize: 22 * fontScale, lineHeight: 24 }}>‹</Text>
              <Text style={{ color: 'rgba(255,255,255,0.85)', fontSize: 15 * fontScale, fontWeight: '600' }}>Voltar</Text>
            </TouchableOpacity>
